...
class Feature(SubModule):
    def __init__(self):
        ...
        # Use timm backbone in features_only mode and consume its feature outputs
        pretrained = True
        # Request a backbone that returns intermediate features. Different timm
        # versions expose internals differently; relying on the backbone call
        # is more robust than accessing attributes like act1/blocks directly.
        self.backbone = timm.create_model('mobilenetv2_100', pretrained=pretrained, features_only=True)
        # Project backbone feature maps to the channel sizes expected by the
        # rest of the network (original implementation expected [24,32,96,160]
        # for the 4 feature levels). Build 1x1 conv projections if necessary.
        try:
            with torch.no_grad():
                ...
                if len(feats) < 4:
                    ...
                    while len(feats) < 4:
                        feats.append(last)
                in_chs = [f.shape[1] for f in feats[-4:]]
        except Exception:
            # fallback to conservative guess if backbone call fails at import
            in_chs = [24, 32, 96, 160]

        target_chs = [24, 32, 96, 160]
        self.proj = nn.ModuleList()
        for inp, out in zip(in_chs, target_chs):
            if inp == out:
                # identity - use nn.Identity for clarity
                self.proj.append(nn.Identity())
            else:
                self.proj.append(nn.Conv2d(inp, out, kernel_size=1, stride=1, padding=0, bias=False))
    ...
    def forward(self, x):
        ...
        if len(feats) < 4:
            ...
            while len(feats) < 4:
                feats.append(last)
        out_feats = feats[-4:]
        # Project to expected channel sizes
        mapped = []
        for f, p in zip(out_feats, self.proj):
            mapped.append(p(f))
        return mapped
