...
def disp_warp(right_input, disparity_samples, padding_mode='border'):
    # Use the tensor's device (works for CPU and CUDA tensors). get_device() returns -1 on CPU
    device = right_input.device
    left_y_coordinate = torch.arange(0, right_input.size()[3], device=device).repeat(right_input.size()[2])
