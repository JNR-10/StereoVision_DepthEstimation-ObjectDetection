...
class BGNet(SubModule):
    ...
    def forward(self, left_input, right_input):         
        ...
        list_float = []
        # Use the tensor's device (works for CPU and CUDA). get_device() returns -1 on CPU
        ...
        N, _, H, W = guide.shape
        #[H,W]
        # Create coordinate grids on the same device as the model tensors
        hg, wg = torch.meshgrid([torch.arange(0, H, device=device), torch.arange(0, W, device=device)])
        # [B,H,W,1]
