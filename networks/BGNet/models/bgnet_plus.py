...
class BGNet_Plus(SubModule):
    ...
    def forward(self, left_input, right_input):        
        ...
        list_float = []
        # Use the tensor's device (torch.device) rather than get_device() which
        # returns -1 for CPU and causes errors when passed to .to().
        ...
        # Move grids to the same device as the model tensors
        hg = hg.to(device)
        wg = wg.to(device)
        #[B,H,W,1]
