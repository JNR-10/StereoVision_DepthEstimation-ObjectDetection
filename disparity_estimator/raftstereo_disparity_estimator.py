...
class RAFTStereoEstimator:
    def __init__(self):
        # build model
        self.model = RAFTStereo(self.get_internal_args())
        ...
        # wrap with DataParallel without explicit device ids
        self.model = torch.nn.DataParallel(self.model)

        # load checkpoint robustly
        try:
            state = torch.load(config.RAFT_STEREO_MODEL_PATH)
        except Exception as e:
            print(f"Warning: failed to load RAFT-Stereo checkpoint normally ({e}). Retrying with map_location='cpu'.")
            state = torch.load(config.RAFT_STEREO_MODEL_PATH, map_location=torch.device('cpu'))
