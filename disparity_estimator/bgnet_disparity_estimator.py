...
class BGNetEstimator:
    def __init__(self):
        # build model
        self.model = BGNet_Plus()
        print(self.model)

        # decide device safely
        desired = getattr(config, 'DEVICE', 'cpu')
        ...
        try:
            checkpoint = torch.load(config.BGNET_PLUS_MODEL_PATH)
        except Exception as e:
            print(f"Warning: failed to load BGNet checkpoint normally ({e}). Retrying with map_location='cpu'.")
            checkpoint = torch.load(config.BGNET_PLUS_MODEL_PATH, map_location=torch.device('cpu'))
    ...
    def load_image(self, left_image, right_image):
        ...
        h1 = int(h1)
        w1 = int(w1)
        # Pillow >=10 removed Image.ANTIALIAS constant; use Resampling.LANCZOS
        try:
            resample = Image.Resampling.LANCZOS
        except AttributeError:
            # older Pillow exposes LANCZOS directly
            resample = Image.LANCZOS

        left_img = left_img.resize((w1, h1), resample=resample)
        right_img = right_img.resize((w1, h1), resample=resample)
