...
class FastACVEstimator:
    def __init__(self):
        ...
        # wrap DataParallel without explicit device_ids so it works on CPU-only machines
        self.model = nn.DataParallel(self.model)

        # load checkpoint robustly (try normal load, fall back to CPU map_location)
        try:
            state = torch.load(config.FASTACV_MODEL_PATH)
        except Exception as e:
            print(f"Warning: failed to load checkpoint normally ({e}). Retrying with map_location='cpu'.")
            state = torch.load(config.FASTACV_MODEL_PATH, map_location=torch.device('cpu'))

        # support different checkpoint dict layouts
        ...
        # load and move model to selected device
