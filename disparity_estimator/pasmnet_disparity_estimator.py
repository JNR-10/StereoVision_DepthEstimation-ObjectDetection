...
class PASMNetEstimator:
    def __init__(self):
        ...
        try:
            if config.PASMNET_MODEL_PATH.split('.')[-1] == 'tar':
                state = torch.load(config.PASMNET_MODEL_PATH)
            else:
                state = torch.load(config.PASMNET_MODEL_PATH)
        except Exception as e:
            print(f"Warning: failed to load PASMNet checkpoint normally ({e}). Retrying with map_location='cpu'.")
            state = torch.load(config.PASMNET_MODEL_PATH, map_location=torch.device('cpu'))
    ...
    def estimate(self, left_image, right_image):
        ...
        disp_ests = self.model(left_img.to(self.device), right_img.to(self.device), max_disp=192)
