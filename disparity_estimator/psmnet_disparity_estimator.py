...
class PSMNetEstimator:
    def __init__(self, type= 'stackhourglass'):
        ...
        # wrap DataParallel without explicit device ids
        self.model = nn.DataParallel(self.model)
        # load checkpoint robustly
        try:
            state = torch.load(config.PSMNET_MODEL_PATH)
        except Exception as e:
            print(f"Warning: failed to load PSMNet checkpoint normally ({e}). Retrying with map_location='cpu'.")
            state = torch.load(config.PSMNET_MODEL_PATH, map_location=torch.device('cpu'))
    ...
    def estimate(self, left_image, right_image):
        ...
        self.model.eval()
        with torch.no_grad():
            output = self.model(imgL.to(self.device), imgR.to(self.device))
