...
def demo():
    ...
    video_writer = None
    if getattr(config, 'VIDEO_SAVE', 0):
        video_out_path = getattr(config, 'VIDEO_PATH', 'output/demo_output.mp4')
        video_fps = getattr(config, 'VIDEO_FPS', 3)
    ...
    for (imfile1, imfile2, calib_file) in tqdm(list(zip(left_images, right_images, calib_files))):
        ...
        if getattr(config, 'VIDEO_SAVE', 0) and video_writer is None:
            h_vid, w_vid = combined_image.shape[:2]
            # fourcc for mp4
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            video_writer = cv2.VideoWriter(video_out_path, fourcc, video_fps, (w_vid, h_vid))
            if not video_writer.isOpened():
                print(f"Warning: failed to open video writer for {video_out_path}")
            else:
                print(f"Recording demo output to {video_out_path} @ {video_fps} FPS")
        # write the combined frame to video if writer available
        if video_writer is not None:
            # ensure frame is uint8
            frame = combined_image.astype(np.uint8)
